-- Tenant profile (dados cadastrais PJ/PF)
create table if not exists public.tenant_profiles (
  ...
  document text, -- CNPJ/CPF (vamos manter genérico)

  -- Responsável
  responsible_name text,
  responsible_document text, -- CPF do responsável (ou documento)
  ...
  -- Regras mínimas
  constraint tenant_profiles_document_required check (document is not null and length(document) >= 5),
  constraint tenant_profiles_email_format check (email is null or position('@' in email) > 1)
);

-- Endereços (fiscal / produção)
...
-- Trigger simples para updated_at (se você já tiver função, pode reaproveitar)
